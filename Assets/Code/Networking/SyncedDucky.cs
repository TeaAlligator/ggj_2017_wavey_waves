...
{
    public class SyncedDucky : NetworkBehaviour
    {
        #region Properties

        [SerializeField] protected Transform PositionSubject;
        [SerializeField] protected Transform RotationSubject;

        #region Setup

        [Header("Setup")] [Range(0, 10)] public int SendRate = 2;
        ...
        [Header("Interpolation")] [Range(0, 1)] public float movementInterpolation = 0.1f;
        ...
        protected void sendInfo()
        {
            if (send)
            {
                if (count == SendRate)
                {
                    count = 0;
                    send = false;
                    Vector3 v = PositionSubject.position;
                    Quaternion q = RotationSubject.rotation;
                    CmdSendPosition(v, q);
                }
                else
                {
        ...
        protected void recontiliation()
        {
            ...
            {
                PositionSubject.position = lastPositionSent;
            }
            if (angle > angleBeforeSnap)
            {
                RotationSubject.rotation = lastRotationSent;
            }
            //prediction
            ...
            PositionSubject.position = vLerp;
            RotationSubject.rotation = qLerp;
        }
